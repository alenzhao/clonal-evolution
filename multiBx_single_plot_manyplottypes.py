...
for i in range(1,r_sweep+1):
	...
	# plt.hist(SIBx, alpha = 0.5, label = str(i))

# print data

bins = np.linspace(0, np.max(data), 100)
# n, bins, patches = P.hist([data[0], data[1], data[2]], 10, histtype = 'bar')
...
'''
#makes a nice, but unreadable multicolored bar chart (dimensions wrong)
